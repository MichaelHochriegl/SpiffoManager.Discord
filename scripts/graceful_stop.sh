...
for x in {$2..2..-1}
do
  tmux send-keys -t $session "servermsg \"Stop in: $x Minuten\"" Enter
  sleep 60
done

for x in {60..10..-10}
do
  tmux send-keys -t $session "servermsg \"Stop in: $x Sekunden\"" Enter
  sleep 10
done

tmux send-keys -t $session  "servermsg \"Stop des Servers jetzt!\"" Enter
sleep 5
./$session stop
sleep 10
git stash
